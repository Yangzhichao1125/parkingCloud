<template>
  <div class="container">
    <a-modal
      ...
      @close="resetFields"
    >
      <template #title> 批量导入-收费规则 </template>
            ...
            <a-row :gutter="80">
              <a-col :span="8">
                ...
                >
                  <a-upload
                    multiple
                    :action="apiHost+'/internetOfThingsAdmin/cfCarPark/importExcelChargingRules'"
                    :headers="header"
                    :data="uploadData"
                    @success="onSuccess"
                    @error="onError"
...
<script lang="ts" setup>
  ...
  const handleClick = (carParkId: any, carType: any) => {
    formData.value.carParkId = carParkId;
    uploadData.value.carParkId = carParkId;
    uploadData.value.carType = carType;
    visible.value = true;
  };
  const resetFields = () => {
