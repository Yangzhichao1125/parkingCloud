...
const radioGroupOptions = ['入场', '出场', '降锁','行驶证录入','环保补录','场内提前缴费码','绑定手机号码'];
...
const radioGroupChange = (e: any) => {
  if (e === '入场') {
    ...
    createExitCodeFormData.value.text =
        `${urlHead.value}/drivingLicense/` +
        createCodeInfo.value.areaId;
  } else if (e === '环保补录') {
    createExitCodeFormData.value.text =
        `${urlHead.value}/scene/epAccessControl/carParkId/` +
        createCodeInfo.value.areaId;
  } else if (e === '场内提前缴费码') {
    createExitCodeFormData.value.text =
        `${urlHead.value}/scene/parkInputFee/carParkId/` +
        createCodeInfo.value.areaId;
  } else if (e === '绑定手机号码') {
    createExitCodeFormData.value.text =
        `${urlHead.value}/pages/pay/scanCode`;
  }
};
