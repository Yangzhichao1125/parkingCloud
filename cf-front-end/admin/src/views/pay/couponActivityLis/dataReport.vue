<template>
  <div class="container">
    <a-modal v-model:visible="visible" :width="1000" draggable :footer="false" @close="resetFields" id="printBox">
      <template #title v-if="!formModel.mainShopId"> {{ modalTitle }} - 优惠券收入报表 </template>
      <template #title v-if="formModel.mainShopId"> {{ mainShopName }} - 关联名下所有优惠券收入报表 </template>
      <a-row>
        <a-col :flex="1">
          <a-form :model="formModel" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }" label-align="left">
            <a-row :gutter="18">
              ...
              <a-col :span="8">
                <a-form-item label="主店铺" field="mainShopId">
                  <a-select
                      v-model="formModel.mainShopId"
                      @change="shopIdChange"
                      placeholder="请选择"
                      allow-clear
                  >
...
import { getDailyIncomeStatisticsListByQuery, flushStatisticsParkDatas, getCfCouponActivityLinkShopList } from '@/api/visualization';
...
const shopIdChange = (shopId: any) => {
  for(const shop of shopIdList.value){
    if(shopId == shop.shopId){
      mainShopName.value = shop.shopName;
      break;
    }
  }
  getList();
...
const getList = async () => {
  ...
  let _shopId = shopId.value;
  if(formModel.value.mainShopId){
    _shopId = '';
  }
...
const getCouponActivityLinkShopList = async () => {
  let newForm = cloneDeep(formModel.value);
  let dateArr = newForm.currentDay.split('-');
  if(newForm.countDateMode=="YYYY-MM"){
    dateArr[2] = '0';
  }
  const { data, code, total } = await getCfCouponActivityLinkShopList({
    params: {
      page: 1,
      size: 200,
      couponActivityId: shopId.value
    },
  });
  if (code == 10002) {
    shopIdList.value = data;
  } else {
    shopIdList.value = [];
  }
};
...
//刷新停车数据报表
const flushStatisticsPark = async () => {
  let newForm = cloneDeep(formModel.value);
  let dateArr = newForm.currentDay.split('-');
  if(newForm.countDateMode=="YYYY-MM"){
    dateArr[2] = '0';
  }
  const { message, mixMessage } = await flushStatisticsParkDatas({
    params: {
      id: shopId.value,
      timeStamp:  formModel.value.minStartTime
    },
  });
  Message.info({
    // @ts-ignore
    content: message + (mixMessage ? '-'+ mixMessage : ''),
    duration: 5 * 1000,
  });
};
