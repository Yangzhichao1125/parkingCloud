                  ...
                  <a-col  :span="8">
                    <a-form-item label="店铺" field="shopId">
                      <a-select
                          ...
                          allow-clear
                      />
                      <a-select v-model="formModel.shopId" :loading="loading" placeholder="输入店铺名" allow-search :allow-clear=true
                  ...
                  <a-col  :span="8">
                    <a-form-item label="下单用户" field="uid">
                      <a-select v-model="formModel.uid" :loading="loading" placeholder="手机号搜索" allow-search :allow-clear=true
                        ...
                        <a-option v-for="item of userList" :key="item?.id" :value="item?.id">{{ item.name || item.stationName }}</a-option>
                      </a-select>
                    </a-form-item>
                  </a-col>
...
<script lang="ts" setup>
  ...
  import {quicklySearchCouponActivity, exportOrderExcel} from "@/api/pay";
  ...
  const handleShopSearch = async (value: any) => {
    if(formModel.value.goodsType=='1' || formModel.value.goodsType=='5' || formModel.value.goodsType=='6' || formModel.value.goodsType=='14'){
      //停车
    ...
    } else if(formModel.value.goodsType=='8' || formModel.value.goodsType=='12' || formModel.value.goodsType=='15'){
      //充电
    ...
    } else if(formModel.value.goodsType=='7'){
      //优惠券充值
  ...
  const downloadOrder = async () => {
    await exportOrderExcel({
      params: checkParams({
        ...
        minPayTime: formModel.value.minPayTime,
        maxPayTime: formModel.value.maxPayTime,
