  ...
  <div class="container">
    <Breadcrumb
      :items="[
        'menu.ucenter',
        'menu.ucenter.user',
        'menu.ucenter.user.subscribeMessageTemplateList',
      ]"
    />
    <a-card class="general-card" :title="$t('menu.ucenter.user.subscribeMessageTemplateList')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
          ...
          >
            ...
            <a-col :span="8">
                <a-form-item label="使用场景" field="messageScenes">
                  <a-select v-model="formModel.messageScenes" placeholder="请选择" allow-clear>
...
<script lang="ts" setup>
  ...
  import { getSubscribeMessageTemplateListByQuery, deleteSubscribeMessageTemplate, cfUserQuicklySearchUser } from '@/api/user';
  ...
  const onDelete = async (id: string) => {
    await deleteSubscribeMessageTemplate({ params: { id } });
    fetchData();
  };
  ...
  const fetchData = async () => {
    loading.value = true;
    // @ts-ignore
    const { data, code, total } = await getSubscribeMessageTemplateListByQuery({
      params: checkParams({
        page: pagination.current,
        size: pagination.pageSize,
        uid: formModel.value.uid,
        scene: formModel.value.messageScenes,
      }),
    });
    loading.value = false;
    if (code === 10002) {
      ...
      for (const i of data) {
        switch(i.messageScenes){
          case 'car_park_package_expire_notice':
            i.newScene = '停车套餐过期提醒';
            break;
          case 'car_charge_notice':
            i.newScene = '车辆充电结束提醒';
            break;
        }
      }
