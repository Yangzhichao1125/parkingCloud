      ...
      if (e.carParkId) {
		...
		methods: {
			...
			intoBillList(plateNo = '') {
				...
				if (plateNo) {
					uni.navigateTo({
						url: '/pages/pay/billList?licensePlateNumber=' + plateNo + '&carParkId=' + this.carParkId
					})
				} else {
					if (this.plateNo) {
						uni.navigateTo({
							url: '/pages/pay/billList?licensePlateNumber=' + this.plateNo + '&carParkId=' + this.carParkId
						})
					} else {
						uni.showToast({
							title: '车牌号为空',
