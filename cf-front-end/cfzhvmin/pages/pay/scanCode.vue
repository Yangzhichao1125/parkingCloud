	...
	export default {
		...
		onLoad(e) {
			...
			if (q) {
				...
				}else if (q.includes('/drivingLicense')) {
					this.scanType = 'drivingLicense';
          this.carpark_id = q.split('/drivingLicense/')[1];
				}else if (q.includes('/epAccessControl/')) {
					this.scanType = 'epAccessControl';
          this.carpark_id = q.split('/scene/epAccessControl/carParkId/')[1];
				}else if (q.includes('/parkInputFee/')) {
          this.scanType = 'parkInputFee';
          this.carpark_id = q.split('/scene/parkInputFee/carParkId/')[1];
        }
			}
			if (token) {
				this.isLogin = true;
				...
				} else if (this.scanType === 'drivingLicense') {
					this.navigateToDrivingLicensePage();
				} else if (this.scanType === 'epAccessControl') {
					this.navigateToEpAccessControlPage();
				} else if (this.scanType === 'parkInputFee') {
          this.navigateToParkInputFeePage();
        }
			} else {
				this.isLogin = false;
			}
      ...
      navigateToDrivingLicensePage() {
				uni.reLaunch({
					url: `/packageAdmin/park/drivingLicense?carParkId=`+this.carpark_id
				});
			},
      navigateToEpAccessControlPage() {
        uni.reLaunch({
          url: `/packageAdmin/park/goodsAdditionalRecording?carParkId=`+this.carpark_id
        });
      },
      navigateToParkInputFeePage() {
        uni.reLaunch({
          url: `/pages/parkPay_input/parkPay_input?carParkId=`+this.carpark_id
        });
      },
			...
			async getCarParkUseLogId() {
				...
				if (code == 10002 && data != null) {
					...
					setTimeout(() => {
						uni.reLaunch({
							url: '/pages/parkPay_input/parkPay_input?carParkId='+data
						})
					}, 1500);
				}
			},
						...
						if (loginRes.code === 24017) {
							...
							switch (_.scanType) {
								...
								case 'bindPhone':
									...
									break;
                case 'drivingLicense':
                  _.navigateToDrivingLicensePage();
                  break;
                case 'epAccessControl':
                  _.navigateToEpAccessControlPage();
                  break;
                  case 'parkInputFee':
                  _.navigateToParkInputFeePage();
                  break;
								default:
									break;
							}
