...
<script>
	import { getStoredCardListByQuery,rechargingStoredCard, deleteCarParkStoredCard } from '../../network/api.js'
		...
		methods: {
			...
			async onDelete(){
				const {
					code,
					message
				} = await deleteCarParkStoredCard(this.applyInfo.id)
				if (code === 10002) {
			...
			async onApply(){
				...
				_appid = wxAppid;
				// #endif
				const { data, code } = await rechargingStoredCard({
					platform:_platform,
			...
			intoAdd(){
				uni.navigateTo({
					url:'/packageMy/my/applyStoredValueCard'
				})
			},
			async getList(){
				const { data, code } = await getStoredCardListByQuery({
					page:1,
					size:200
				})
				if(code === 10002){
